...
<?php
        ...
        $fechasiguiente = date('Y-m-d', strtotime($_REQUEST['fecha'] . ' + 1 days'));
        if ($resultado = mysqli_query($recurso, "SELECT * FROM pedidos WHERE Fecha > '{$_REQUEST['fecha']}' AND Fecha < '{$fechasiguiente}'"))
        {
...
?>
        ...
        <form method="post" action="bddetalle.php">
            <tr>
                <td><input name="codigopedido" value= <?php
                            echo $fila2[0];
?>></td>
                <td><input name="codigoproducto" value= <?php
                            echo $fila2[1];
?>></td>
                <td><input name="unidades" value= <?php
                            echo $fila2[2];
?>></td>
                <td><input name="preciounitario" value= <?php
                            echo $fila2[3];
?>></td>
                <td></td>
